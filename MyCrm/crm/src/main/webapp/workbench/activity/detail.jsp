			...
			$.ajax({
				...
				success:function (data) {
                    if(data.success){
                    	...
                    	//添加备注成功后清空textarea
						$("#remark").val("");
                    	//添加成功在textarea文本域上方新增一个div
						/*
						   data
						     {"success":true/false,"aRemark":{备注信息}}
						 */
						var html="";
						...
						html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activity.name}</b> <small style="color: gray;">'+data.aRemark.createTime+' 由 '+data.aRemark.createBy+'</small>';
						html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
						html += '<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+data.aRemark.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>';
						html += '	&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	function showRemarkList() {
		$.ajax({
			...
			success:function (data) {
                ...
                /*
                   javascript:void(0)
                    这句话是禁用超链接，只能呢个使用触发事件的方式来操作

                    在遍历当中我们动态生成的元素，通常直接触发事件，即将触发到的时间写在动态生成的元素中
                 */
                /*
                  id="'+n.id+'" 这个id是我们自己加上去的，我们以后要操作动态生成的元素
                  通常都采用使用 id 的形式，注意格式
                 */
                ...
                //在$("#remarkDiv")这个jQuery对象上面追加新的对象
				$("#remarkDiv").before(html);
			}
		})
	}
				...
				success:function (data) {
					if(data.success){
						...
						/*
						  使用删除成功后，刷新备注列表的方法不行，
						  showRemarkList();
						  因为记录使用的before的方法，页面上都会保留原有的数据
						 */
						//应该这样：删除成功后移除该动态生成div
						$("#"+id).remove();
