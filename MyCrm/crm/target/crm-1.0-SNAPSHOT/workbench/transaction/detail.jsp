...
<%
    ...
    for(int i=0;i<dvList.size();i++){
        //使用for循环遍历dvList(字典值)，先取得每一个dicValue
...
<script type="text/javascript">
       ...
       $.ajax({
           ...
           success:function (data) {
                ...
                if(data.success){
                    ...
                    //改变阶段成功后，将所有的阶段图标重新判断，重新赋予图标和颜色
                    changeIcon(stage,i);
                }else{
                    alert("修改阶段失败");
                }
    ...
    function changeIcon(stage,index1) {
        ...
        //如果当前阶段的可能性为0，前7个是黑圈，后两个一个是红叉一个是黒叉，但不确定位置
        if(currentPossibility=="0"){
            ...
            //如果当前阶段可能性不为0，当前可能性之前是绿圈，当前之后为黑圈，后两个一定为黒叉
            //遍历前7个
            for(var i=0;i<point;i++){
                //如果是当前阶段
                if(i==index){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
        ...
        <%
            ...
            /*
                 判断当前阶段
                 绿圈表示已完成，绿豆表示当前阶段，黑圈表示未完成阶段，黒叉表示线索没有丢失
                 红叉表示在此阶段线索丢失，可能性为0
            ...
            if("0".equals(currentPossibility)){
                  for(int i=0;i<dvList.size();i++){
                      ...
                      //取得每一个便利出来的阶段，根据每一个遍历出啦的阶段取其可能性
                ...
                for(int i=0;i<dvList.size();i++){
                    ...
                    if("0".equals(listPossibility)){
                        ...
                        //如果遍历出来的阶段的可能性不为0，说明是前七个阶段，绿圈，绿豆，黑圈
                        if(i == index){
                            //如果是当前阶段，为绿豆
                           %>
